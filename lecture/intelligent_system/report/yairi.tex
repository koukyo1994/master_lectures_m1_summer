...
音声データとしては、研究室の研究会の様子を録音したもの、約160分程度のものを使用し、潜在状態の定義は、
発表者交代、発表中、質問中の3状態としました。なお、今回は上記のような潜在状態を仮定していますが、教師なし
モデルとして学習をさせたため、実際には先述の3状態ではない状態がクラスタとして現れている可能性があります。\\

このテーマを選択した理由として、個人的に以前から研究会の文字起こしをしたいと考えており、特に話者を識別して
アノテーションを行いたいと考えていたのですが、その前段階としてデータの生起する文脈を識別することができれば、
アノテーションのコストが減らせたりより純粋な学習データを入手できるのではないかと考えたから、というものが挙げ
られます。
\section{}
